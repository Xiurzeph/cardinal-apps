...
var app = initializeApp(myFirebaseConfig);
var auth = getAuth(app);
var db = getFirestore(app);
var appId = "cardinal-lookup";
var currentUser = null;
var currentBatchId = null;
var currentBatchSource = "private";
var pendingAction = null;
var groupedBatch = [];
var currentTab = "formatter";
var currentDbSubTab = "private";
var activeShareText = "";
var activeShareGroupIndex = null;
var addressCache = /* @__PURE__ */ new Map();
var addressStatuses = /* @__PURE__ */ new Map();
var addressStatusSaveHandle = null;
var addressStatusSaveTimeout = null;
var ADDRESS_STATUS_SAVE_IDLE_MS = 2e3;
var ADDRESS_STATUS_SAVE_FALLBACK_MS = 1e3;
var lastSyncedStatuses = /* @__PURE__ */ new Map();
var addressStatusSyncInProgress = false;
var ADDRESS_STATUSES = [
  "home",
  "letter",
  "busy",
  "visited",
...
function getStatusStyle(status) {
  switch (status) {
    ...
    case "visited":
      return { textColor: "text-teal-700", bgColor: "bg-teal-50", strikethrough: true, indicator: "\u{1F463} Visited" };
    case "not home 1":
      return { textColor: "text-gray-900", bgColor: "bg-yellow-50", strikethrough: false, indicator: "\u{1F6AA} Not Home 1" };
    case "not home 2":
      return { textColor: "text-gray-900", bgColor: "bg-yellow-100", strikethrough: false, indicator: "\u{1F6AA} Not Home 2" };
    case "not home 3":
      return { textColor: "text-gray-900", bgColor: "bg-white", strikethrough: false, indicator: "\u{1F6AA} Not Home 3" };
    case "no soliciting":
    ...
    case "do not call":
      return { textColor: "text-white", bgColor: "bg-red-500", strikethrough: true, indicator: "\u{1F4DE} Do Not Call" };
    default:
      return { textColor: "text-gray-900", bgColor: "bg-white", strikethrough: false, indicator: "" };
  }
}
...
var MD_GEODATA_URL = "https://mdgeodata.md.gov/imap/rest/services/PlanningCadastre/MD_PropertyData/MapServer/0/query";
var MD_LOCATOR_URL = "https://mdgeodata.md.gov/imap/rest/services/GeocodeServices/MD_MultiroleLocator/GeocodeServer/findAddressCandidates";
...
async function fetchPropertyData(addressStr) {
  try {
    ...
    const geocodeParams = new URLSearchParams({
      ...
      outSR: 102100,
      outFields: "County"
      // Explicitly request the County attribute
    });
...
function renderResults(groups) {
  ...
  const html = groups.map((group, gIdx) => {
    ...
    const itemsHtml = group.items.map((item) => {
      ...
      const statusIndicator = status !== "home" ? `<div class="text-[9px] font-bold uppercase mt-2 opacity-75">${styleInfo.indicator}</div>` : "";
...
window.completeAndCloseShare = async function() {
  if (activeShareGroupIndex !== null && activeShareGroupIndex !== void 0) {
...
window.saveOrUpdateBatch = function() {
  ...
  if (modal && input) {
    input.value = `Batch ${(/* @__PURE__ */ new Date()).toLocaleDateString()}`;
...
window.confirmSave = async function() {
  ...
  try {
    ...
    for (const [k, v2] of Object.entries(payload.addressStatuses || {})) {
      lastSyncedStatuses.set(k, v2);
    }
...
async function loadHistory() {
  ...
  try {
    ...
    let batches = [];
    querySnapshot.forEach((doc2) => batches.push({ id: doc2.id, ...doc2.data() }));
    batches.sort((a, b2) => b2.timestamp - a.timestamp);
    if (batches.length === 0) {
    ...
    const batchesHtml = batches.map((batch) => {
      const creatorLine = currentDbSubTab === "public" ? `<div class="text-[10px] text-gray-400 font-bold uppercase mt-1">Shared by: ${batch.createdBy || "Unknown"}</div>` : "";
